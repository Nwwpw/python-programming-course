...
def welcome_message(name, course):
    ...
    return f"Welcome {name} to {course} class!" #return "Welcome + name + to + course + class!"(แบบนี้ก็ได้)
...
def calculate_circle(radius):
    ...
    pi = 3.14159 #กำหนดค่าตามโจทย์
    area = pi * radius * radius #คำนวณพื้นที่วงกลม πr^2
    circumference = 2 * pi * radius #คำนวณเส้นรอบวง 2πr
    #ปัดทศนิยมสองค่าให้เหลือ 2 ตำแหน่ง
    area = round(area,2) #round()ใช้ปัดทศนิยม
    circumference = round(circumference,2) #round()ใช้ปัดทศนิยม
    return {"area": area, "circumference": circumference} #return dictionary ที่มี area และ circumference
...
def analyze_scores(scores):
    ...
    total = sum(scores)                      # ผลรวมคะแนนทั้งหมด
    count = len(scores) 
    average = total / count                  # คะแนนเฉลี่ย ปัดทศนิยม 1 ตำแหน่ง
    highest = max(scores)                    # คะแนนสูงสุด
    lowest = min(scores)                     # คะแนนต่ำสุด
    counter = 0
...
def count_vowels_consonants(text):
    ...
    text.lower()    #ทำให้ทุกตัวเป็นพิมพ์เล็ก
    text.replace(" ","")    #ลบช่องว่าง(มันจะติดกันเลย ไม่มีเว้น)
    #ลบเลข0-9ออกจากข้อความ
    text = text.replace("0","")
    text = text.replace("1","")
    text = text.replace("2","")
    text = text.replace("3","")
    ...
    text = text.replace("8","")
    text = text.replace("9","")
    vowels = text.count('a') + text.count('e') + text.count('i') + text.count('o') + text.count('u')    #นับจำนวนสระทั้งหมดในข้อความ
    consonants = len(text) - vowels     #นับจำนวนพยัญชนะ โดยเอาจำนวนอักขระทั้งหมดลบด้วยจำนวนสระ
